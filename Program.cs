...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            {
                case "1":
                    await ControlFan(client);
                    break;
                case "2":
                    await ControlHeater(client);
                    break;
                case "3":
    ...
    static async Task DisplayState(HttpClient client)
    {
    ...
    static async Task ControlFan(HttpClient client)
    {
    ...
    static async Task ControlHeater(HttpClient client)
    {
    ...
    static async Task ReadTemperature(HttpClient client)
    {
    ...
    static async Task ShutdownSimulation(HttpClient client)
    {
        Console.WriteLine("Shutting down the simulation...");
        try
        {
            var response = await client.PostAsync("api/simulation/shutdown", null);
            if (response.IsSuccessStatusCode)
            {
                Console.WriteLine("Simulation successfully shut down.");
            }
            else
            {
                Console.WriteLine($"Failed to shut down the simulation: {response.ReasonPhrase}");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error shutting down the simulation: {ex.Message}");
        }
    }
